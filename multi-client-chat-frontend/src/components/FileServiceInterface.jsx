...
export default function FileServiceInterface({ service }) {
  ...
  React.useEffect(() => {
    ...
    const connect = () => {
      if (isConnecting.current || !isMounted) return;
    ...
    return () => {
      isMounted = false;
      clearTimeout(timer);
      // Don't close WebSocket immediately - React Strict Mode calls this even when component is still mounted
      // Only close after a delay to allow operations to complete
      setTimeout(() => {
  ...
  const connectToApiGateway = () => {
    try {
      ...
      const socket = new WebSocket('ws://localhost:9001/api');
      ...
      socket.onmessage = (event) => {
        ...
        switch (response.type) {
          ...
          case "SERVICE_STATUS_UPDATE":
            // Ignore service status updates (broadcast from API Gateway)
      ...
      socket.onclose = (event) => {
        ...
        if (!event.wasClean) {
          console.log('Connection lost unexpectedly, will reconnect in 2 seconds...');
          setTimeout(() => {
            connectToApiGateway();
          }, 2000);
        }
